...
var vPosition, vBuffer, bufferID, thetaStop = [-160.0, 0.0, 0.0], rotAxis = false, offset = 0, z = 0.0, saveLength = 0, objectTexture = 0, objText;
...
var sphereTextureBind, animate = true, eyeTextureBind;

var near = 0.01, far = 110.0, radius = 1.0, thetaCam  = 0.1, phi = 0.0, eye, Num = 1, program, sphereVec, planeTextureBind , animalTextureBind;
...
window.onload = function init()
{
	...
	objText = gl.getUniformLocation(program, "objectTexture");
	...
	imageAn.onload = function() {
		animalTextureBind = configureTexture( imageAn );
	}
	
	imageEye.onload = function() {
		eyeTextureBind = configureTexture( imageEye );
	}
	...
	window.onkeydown = function (event){
		...
		if(translate === 'W') {
			...
			//up
			if(rotZlowArm <= 7) rotZlowArm += 1.0;
		}
		else if(translate === '('){
			//down
			if(rotZlowArm >= -7) rotZlowArm -= 1.0;
...
function render()
{
	...
	objectTexture = 0;
	gl.uniform1i(objText, objectTexture);
	...
	gl.drawArrays( gl.TRIANGLES, index, offset );
	
	//animal
	objectTexture = 2;
	gl.uniform1i(objText, objectTexture);
	
	gl.bindTexture(gl.TEXTURE_2D, animalTextureBind);
	createAnimal();
